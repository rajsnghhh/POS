...
export class CustomerDetailComponent extends BaseComponent implements OnInit {
  ...
  patchCustomer() {
    this.customerForm.patchValue({
      ...
      aadharCard:this.customer.aadharCard,
      dependantCard:this.customer.dependantCard,
      serviceNo:this.customer.serviceNo,
      category:this.customer.category,
      countryName: this.customer.countryName
    });
    this.Approve = this.customer.isVarified;
  }
    ...
    this.sub$.sink = this.customerService
      ...
      .subscribe((c) => {
        ...
        if (c) {
          ...
          this.existingMobileNo = true;
        } else {
          mobileNoControl.setValidators([Validators.required]);
          this.existingMobileNo = false;
        }
  ...
  createBuildForm(): Customer {
    const customerObj: Customer = {
      ...
      OTP: '1234',
      Password: '1234',
      isVarified: this.Approve,
      isUnsubscribe: false,
      aadharCard: this.customerForm.get('aadharCard').value,
      dependantCard: this.customerForm.get('dependantCard').value,
      serviceNo:this.customerForm.get('serviceNo').value,
      category: this.customerForm.get('category').value,
