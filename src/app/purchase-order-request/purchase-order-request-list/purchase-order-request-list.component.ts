  ...
  onDownloadPO() {
    ...
    this.purchaseOrderService.getAllPurchaseOrder(this.purchaseOrderResource)
      .subscribe(data => {
        ...
        let PODetails = [];
        this.downloadPO.forEach((data: any) => {
          PODetails.push({
        ...
        let workSheet = XLSX.utils.sheet_add_json(workBook, PODetails, { origin: "A2", skipHeader: true });
        XLSX.utils.book_append_sheet(workBook, workSheet, this.translationService.getValue('PURCHASE_ORDERS_REQUEST'));
        XLSX.writeFile(workBook, this.translationService.getValue('PURCHASE_ORDERS_REQUEST') + ".xlsx");
      });
  }
}
