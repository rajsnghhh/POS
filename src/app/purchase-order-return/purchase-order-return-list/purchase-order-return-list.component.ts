...
export class PurchaseOrderReturnListComponent extends BaseComponent {
  ...
  displayedColumns: string[] = ['action', 'poCreatedDate', 'orderNumber', 'deliveryDate', 'supplierName', 'totalDiscount', 'totalTax', 'totalAmount', 'totalPaidAmount', 'paymentStatus','returnPayment','returnType'];
  filterColumns: string[] = ['action-search', 'poCreatedDate-search', 'orderNumber-search', 'deliverDate-search', 'supplier-search', 'totalAmount-search', 'totalDiscount-search', 'totalTax-search', 'totalPaidAmount-search', 'paymentStatus-search','returnPayment-search','returnType-search'];
    ...
    this.purchaseOrderService.getAllPurchaseOrder(this.purchaseOrderResource)
      .subscribe(data => {
        ...
        let heading = [[
          ...
          this.translationService.getValue('Return Payment'),
          this.translationService.getValue('Return Type'),
        ...
        this.downloadGRN.forEach((data: any) => {
          GRNDetails.push({
            ...
            'Total Amount': data.totalReturnAmount,
            ...
            'Return Payment': data.purchaseOrderPaymentStatus,
            'Return Type': data.purchaseOrderReturnType
          });
        });
        ...
        XLSX.utils.book_append_sheet(workBook, workSheet, this.translationService.getValue('GRN Return List'));
        XLSX.writeFile(workBook, this.translationService.getValue('GRN Return List') + ".xlsx");
      });
  }
}