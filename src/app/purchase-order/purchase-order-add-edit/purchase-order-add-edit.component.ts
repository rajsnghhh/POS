...
export class PurchaseOrderAddEditComponent extends BaseComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.purchaseOrder?.deliveryStatus) {
      this.deliverStatus = this.purchaseOrder?.deliveryStatus.toString();
    }
  }
  ...
  getBatch() {
    this.purchaseOrderService.getBatch().subscribe(data => {
      ...
      .subscribe((purchaseOrderData: { 'purchaseorder': PurchaseOrder }) => {
        ...
        if (this.purchaseOrder) {
          ...
          this.purchaseOrderForm = this.fb.group({
            ...
            batchNo: [this.purchaseOrder.batchNo, [Validators.required]],
          ...
          this.purchaseOrderForm = this.fb.group({
            ...
            invoiceNo: [''],
            purchasePaymentType: ['Credit', [Validators.required]],
            filerSupplier: [''],
  ...
  createPurchaseOrderItemPatch(index: number, purchaseOrderItem: PurchaseOrderItem) {
    ...
    const formGroup = this.fb.group({
      ...
      salesPrice: [purchaseOrderItem.salesPrice, [Validators.required]],
      margin: [purchaseOrderItem.margin],
      expDate: [purchaseOrderItem.expDate?.substring(0, purchaseOrderItem.expDate.indexOf('T'))],
      mrp: [purchaseOrderItem.mrp, [Validators.required]],
      quantity: [purchaseOrderItem.quantity, [Validators.required]],
      taxValue: [taxs],
      stock: [purchaseOrderItem.stock],
      ...
      purchaseOrderItems.forEach((po, index) => {
        if (po.unitPrice && po.quantity) {
          ...
          this.purchaseOrderItemsArray.controls[index].patchValue({
            // salesPrice: ((po.unitPrice * po.margin) / 100 + po.unitPrice).toFixed(2),
            // const roundedTotalAmount = parseFloat(this.customRoundPipe.transform(sale.totalAmount));
            salesPrice: parseFloat(this.customRoundPipe.transform(((po.unitPrice * po.margin) / 100 + po.unitPrice).toFixed(2)))

          });
          const gradsaleTotal = this.salesGrandTotal + (this.purchaseOrderItemsArray.controls[index].value.salesPrice * po.quantity);
          this.salesGrandTotal = parseFloat(this.customRoundPipe.transform(parseFloat(gradsaleTotal.toFixed(2))));
  ...
  onUnitPriceChange(purchaseOrderItem: any, index: any) {
    let errorDisplayed = false;

    purchaseOrderItem.valueChanges.pipe(
      debounceTime(300) // Adjust the debounce time as needed
    ).subscribe(value => {
      if (value.mrp < value.salesPrice && !errorDisplayed) {
        this.toastrService.warning(this.translationService.getValue('MRP should not be less than sales price'));
        errorDisplayed = true; // Set the flag to true to indicate that the error has been displayed
        this.purchaseOrderItemsArray.controls[index].patchValue({
          mrp: value.salesPrice
        });
      } else if (value.mrp >= value.salesPrice) {
    ...
    if (unit.value) {
      switch (unit.operator) {
        case Operators.Plush:
          ...
          mrpprice = product.mrp + parseFloat(unit.value);
          marginpercent = product.margin + parseFloat(unit.value);
          salesData = product.salesPrice + parseFloat(unit.value);
          break;
  ...
  onProductSelectionChange(productId: any, index: number, isFromUI = true) {
    ...
    if (product) {
      ...
      if (isFromUI) {
        this.purchaseOrderItemsArray.controls[index].get('filterProductValue').patchValue('');
      } else {
        this.purchaseOrderItemsArray.controls[index].get('productId').patchValue(productId);
      }
    }
    this.purchaseOrderItemsArray.controls[index].patchValue({
      unitPrice: product.purchasePrice,
        ...
        purchaseOrder.purchaseOrderItems.push(
          {
            ...
            mrp: parseFloat(po.mrp),
            expDate: po.expDate,
            margin: parseFloat(po.margin),
            salesPrice: parseFloat(po.salesPrice),
            stock: po.stock,
