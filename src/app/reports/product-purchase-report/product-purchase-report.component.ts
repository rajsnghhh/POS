  ...
  onDownloadReportPdf() {
    this.purchaseOrderService.getAllPurchaseOrderItemReport(this.purchaseOrderResource)
      .subscribe((c: HttpResponse<PurchaseOrderItem[]>) => {
        ...
        const purchaseOrderReport = this.purchaseOrderItems.map((purchaseOrder: PurchaseOrderItem, index: number) => [
          index + 1,
          purchaseOrder.productName,
          purchaseOrder.purchaseOrderNumber,
          purchaseOrder.supplierName,
          this.utcToLocalTime.transform(purchaseOrder.poCreatedDate, 'shortDate'),
          purchaseOrder.unitName,
          purchaseOrder.unitPrice,
          purchaseOrder.quantity,
          purchaseOrder.discount,
          purchaseOrder.purchaseOrderItemTaxes.map(c => c.taxName + '(' + c.taxPercentage + ' %)',),
          purchaseOrder.taxValue,
          ((purchaseOrder.unitPrice * purchaseOrder.quantity) - purchaseOrder.discount + purchaseOrder.taxValue).toFixed(2)
        ]);

        const totalAmount = this.purchaseOrderItems.reduce((sum, order) => sum + ((order.unitPrice * order.quantity) - order.discount +
          order.taxValue), 0);
        const totalRow = [
          '',
          '',
        ...
        const headerText = 'Product Purchase Report';
        ...
        doc.save(this.translationService.getValue('PURCHASE_ORDER_REPORT') + '.pdf');
      });
  }
}
