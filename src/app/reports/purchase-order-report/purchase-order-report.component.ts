  ...
  onDownloadReportPdf() {
    this.purchaseOrderService.getAllPurchaseOrderReport(this.purchaseOrderResource)
      .subscribe((c: HttpResponse<PurchaseOrder[]>) => {
        ...
        const purchaseOrderReport = this.purchaseOrders.map((purchaseOrder: PurchaseOrder, index: number) => [
          index + 1,
          ...
          purchaseOrder.totalPaidAmount,
          this.paymentStatusPipe.transform(purchaseOrder.paymentStatus),
          purchaseOrder.status == 1 ? 'True' : 'False'
        ]);

        const totalAmount = this.purchaseOrders.reduce((sum, order) => sum + order.totalAmount, 0);
        const totalRow = [
