...
import { SalesOrderPayment,DeliveryDetails } from '@core/domain-classes/sales-order-payment';
...
export class SalesOrderPaymentService {
  ...
  assignDeliveryPerson(deliveryDetails: DeliveryDetails): Observable<DeliveryDetails | CommonError> {
    const url = `SalesOrder/UpdateSalesOrderStatus`;
    return this.http.put<DeliveryDetails>(url, deliveryDetails)
      .pipe(catchError(this.commonHttpErrorService.handleError));
  }
