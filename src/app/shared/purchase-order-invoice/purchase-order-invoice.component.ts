...
export class PurchaseOrderInvoiceComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['purchaseOrder']) {
      ...
      this.totalsalesValueCount(this.purchaseOrderItems);
    }
    setTimeout(() => {
      this.printInvoice();
    }, 1000);
  ...
  totalsalesValueCount(saledata:any){
    let saleValue = 0
    for(let sale of saledata){
       saleValue=saleValue+(sale.quantity * sale.salesPrice);
    }
    this.saleAmount=saleValue;
  }
