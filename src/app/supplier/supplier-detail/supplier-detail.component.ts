...
export class SupplierDetailComponent extends BaseComponent implements OnInit {
    ...
    const routeSub$ = this.route.data.subscribe(
      (data: { supplier: Supplier }) => {
        if (data.supplier) {
          ...
          this.supplierService.getSupplierDoccuments(data.supplier.id).subscribe(val=>{
            this.myFiles=val;
          })
  ...
  getFileDetails(e) {
    for (var i = 0; i < e.target.files.length; i++) {
      ...
      let obj = {
        ...
        fileExtension: '.' + e.target.files[i].name.split('.').pop(),
        isImageChanged: true,
        documentData: ''
      }
      ...
      reader.onload = (_event) => {
        obj.documentData = reader.result.toString();
      }
  ...
  patchSupplier() {
    this.supplierForm.patchValue({
      ...
      supplierNo: this.supplier.supplierNo,
      ...
      panNo: this.supplier.panNo,
      udyogAadhar: this.supplier.udyogAadhar,
      msme: this.supplier.msme,
        ...
        this.sub$.sink = this.supplierService
          ...
          .subscribe((c) => {
            ...
            for (let i = 0; i < this.myFiles.length; i++) {
              this.myFiles[i].supplierId = c.id;
            }
            let param = {
              supplierDocuments: this.myFiles
            }
            this.supplierService.addSupplierDoccument(param).subscribe(data => {
            })
  ...
  createBuildForm(): Supplier {
    const supplierObj: Supplier = {
      ...
      supplierNo: this.supplierForm.get('supplierNo').value,
      supplierType: this.supplierForm.get('supplierType').value,
      ...
      panNo: this.supplierForm.get('panNo').value,
      udyogAadhar: this.supplierForm.get('udyogAadhar').value,
      msme: this.supplierForm.get('msme').value,
      gstNo: this.supplierForm.get('gstNo').value,
      pin: this.supplierForm.get('pin').value
    };
    return supplierObj;
  }
}
