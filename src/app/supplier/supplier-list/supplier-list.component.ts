...
export class SupplierListComponent extends BaseComponent implements OnInit {
  ...
  onDownloadSupplier() {
    this.supplierService.downloadSuppliers(this.supplierResource)
      .subscribe(data => {
        this.downloadSupplier = data.body;
        ...
        let heading = [[
          ...
          this.translationService.getValue('COUNTRY'),
          this.translationService.getValue('CONTACT_PERSON'),
        ]];

        let supplierDetails = [];
        this.downloadSupplier.forEach((data: any) => {
        ...
        let workSheet = XLSX.utils.sheet_add_json(workBook, supplierDetails, { origin: "A2", skipHeader: true });
        XLSX.utils.book_append_sheet(workBook, workSheet, this.translationService.getValue('SUPPLIER_LIST'));
        XLSX.writeFile(workBook, this.translationService.getValue('SUPPLIER_LIST') + ".xlsx");
      });
  }
